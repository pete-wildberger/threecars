...
export class App {
  ...
  addWorld = () => {
    const sides: number = 40;
    const tiers: number = 40;
  ...
  addPathTree = () => {
    ...
    let lane: number = Math.floor(Math.random() * 3);
  ...
  addWorldTrees = () => {
    let numTrees: number = 36;
    const gap: number = 6.28 / 36;
  ...
  addTree = (inPath, row, isLeft?) => {
    ...
    if (inPath) {
      ...
      this.sphericalHelper.set(
        this.worldRadius - 0.3,
        this.pathAngleValues[row],
  ...
  update = () => {
    ...
    this.heroSphere.position.x = THREE.Math.lerp(
      this.heroSphere.position.x,
      this.currentLane,
      2 * this.clock.getDelta()
